...
export async function getTimelineEventsByCaseId(
  ...
  if (result.error || !result.data?.length) {
    ...
    return mockResult(
      sortTimeline(developmentTimelineEvents.filter((event) => event.case_id === caseId)),
...
export async function getTimelineEventsByPatientId(
  ...
  if (result.error || !result.data?.length) {
    ...
    return mockResult(
      sortTimeline(developmentTimelineEvents.filter((event) => event.patient_id === patientId)),
...
export async function getTimelineEventsByOrganization(
  ...
  if (result.error || !result.data?.length) {
    ...
    return mockResult(
      sortTimeline(developmentTimelineEvents.filter((event) => event.organization_id === organizationId)),
